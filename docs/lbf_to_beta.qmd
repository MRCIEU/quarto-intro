...
Is it possible to convert BF to beta and standard error? According to [@giambartolomei2014]
...
@fig-beautiful-figure looks good - it's setting different values to 0 in the two lbf vectors that correspond to two causal variants

```{r}
p <- ggplot(ss, aes(x=pos, y=fval)) +
  geom_point()
ggplotly(p)
```
